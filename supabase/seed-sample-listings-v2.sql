-- Sample Listings Data for Supabase (Version 2)
-- This script creates sample property listings using existing user profiles
-- Run this in Supabase SQL Editor after you have at least one user registered
...
-- Create sample properties using an existing user profile
-- This script will use the first available seller or any user as the property owner
DO $$
DECLARE
  namibia_id UUID;
...
BEGIN
  ...
  -- Try to find an existing user profile (preferably a seller, but any will do)
  SELECT id INTO seller_id FROM profiles 
  WHERE user_type = 'seller' 
  LIMIT 1;
  
  -- If no seller found, use any existing profile
  IF seller_id IS NULL THEN
    SELECT id INTO seller_id FROM profiles 
    LIMIT 1;
  END IF;
  
  -- Only proceed if we have a user to work with
  IF seller_id IS NOT NULL THEN
    ...
    -- Add additional interior images (only if they don't exist)
    INSERT INTO property_images (property_id, url, alt_text, order_index)
    SELECT 
      p.id,
    ...
    WHERE p.seller_id = seller_id
    AND NOT EXISTS (
      SELECT 1 FROM property_images pi WHERE pi.property_id = p.id AND pi.order_index = 1
    );
    ...
    WHERE p.seller_id = seller_id
    AND NOT EXISTS (
      SELECT 1 FROM property_images pi WHERE pi.property_id = p.id AND pi.order_index = 2
    );
    ...
    WHERE p.seller_id = seller_id
    AND NOT EXISTS (
      SELECT 1 FROM property_images pi WHERE pi.property_id = p.id AND pi.order_index = 3
    );
    ...
    RAISE NOTICE 'Sample properties created successfully for user ID: %', seller_id;
  ELSE
    RAISE NOTICE 'No user profiles found. Please register at least one user first, then run this script again.';
  END IF;
END $$;
